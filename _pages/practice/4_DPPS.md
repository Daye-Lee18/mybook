...
해당 문제에서 중요한 것은 operation 3번, ***현재 node에서 all of the descendants와 ancestors를 traverse 하거나 혹은 그들의 상태를 파악하고 있어야 한다.***

- ancestors를 traverse하는 방법은 무엇일까? descendant와 다르게 ancestors는 각 노드당 1개씩만 존재하므로 depth가 최대 D이면 해당 traversal은 최대 O(D)밖에 걸리지 않는다. 따라서, while문을 이용하여 문제를 풀 수 있다. 
- 이와 달리, descendant를 구하는 것은 stack구조를 사용한다. direct descendant에 접근해, while문을 이용하여 그들의 descendant에 접근하는 방식이다. 이는 트리의 size가 S일때 최대 O(S)만큼 걸린다. 
    - 방법 1: 스택 DFS / 큐 BFS 
        - 인접 리스트 (children)을 만들어두고 
        - stack으로 DFS 돌면서 locked descendant를 찾고, upgrade이면 unlock까지 진행 
        - 시간: O(#desc)
        - 공간: O(#desc) (스택/큐)
        - 구현이 쉬운 장점이 있으나, 최악의 경우 트리 전체의 사이즈인 O(N)이 걸림. 
        - 참고로 "재귀 DFS"도 동일한 복잡도이지만, 파이썬에선 recursion depth 위험이 있어서 실질적으로 stack이 안전하다. 
    - 방법2: "locked descendant set"을 유지해서 탐색을 줄이는 방법 (방법1보다 더 빠를 수 있음)
        - 해당 문제의 operation 3이 요구하는 건 "모든 자손을 보라"는 것이 아니라, 
        - 자손 중 lock된 노드가 있는지 + 그 Lock 을 풀기 
        - 하지만, subtree의 노드를 다 봐야하는 것은 마찬가지이므로, 방법 1에서 말한 것과 다를바가 없음 
    - 방법3: Euler tour + Fenwick/Segment Tree (상태 "파악"은 빠르게, "해제"는 여전히 비용)
        - 트리를 오일러 투어로 펼치면 subtree가 구간이 되니까
            - "자손 중 lock이 있는지?" -> 구간합으로 O(logN) 체크 가능하다. 
        - 그러나 upgrade operation은 "자손의 lock을 전부 해제" 해야해서, 
            - lock된 노드들을 실제로 찾아서 꺼내야한다. 
            - 결국 unlock 대상 개수 K만큼을 처리해야해서 O(K)는남는다. 
        - 즉, 존재 여부 체크는 빨라지지만, 일괄 해제 때문에 완전한 로그 시간은 아님. 
````

````{admonition} Euler tour + Fenwick (BIT)
:class: dropdown 

- 목적: subtree의 구간 빠르게 확인하기! 
- 핵심 아이디어: 
    - 트리를 Euler tour로 펼치면, 어떤 노드 u의 subtree가 배열 구간 `[tin[u], tout[u]]`가 됨. 
    - 
````
...
```{literalinclude} ../solutions/DPPS/14.py
:language: python 
