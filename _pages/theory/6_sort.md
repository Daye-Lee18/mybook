...
슬라이싱(slicing) 이란, 리스트의 일부 구간을 잘라 새로운 리스트를 만드는 것이다. 형식은 다음과 같다. 
...
|---|---|---|---|---|
|start| 시작 인덱스|0 | len(arr) -1 | 포함됨 | 
|end | 끝 경계 | len(arr) | -1 | 포함되지 않음 | 
|step | 간격 (양수면 오른쪽, 음수면 왼쪽으로 이동) | 1 | -1 | 방향 | 

- start와 end가 생략되면 **step의 부호에 따라 기본값이 달라진다.**
- `end`값는 "인덱스"가 아니라 *포함되지 않는 경계 (exclusive boundary)"로 해석된다. 
...
두 표현은 비슷해 보이지만 결과가 다르다.

`arr[::-1]` 은 다음과 같이 **step < 0 규칙에 따라 기본값이 설정된다.**
...
하지만 여기서 중요한 점은  
**생략된 값과 직접 작성한 값이 동일하게 처리되지 않는다는 것**이다.

Python은 슬라이싱을 실행하기 전에 내부적으로  
**slice normalization (또는 index adjustment)** 을 수행한다.
...
처럼 start와 end가 생략된 경우에는  
Python 내부 로직이 전체 리스트를 역순으로 순회하도록  
적절한 범위를 계산한다.
...
과 같은 상태가 되어 반복이 시작되지 않으며  
결과적으로 **빈 리스트 []** 가 반환된다.

---

## slicing에서의 자동 범위 보정 (clamping)

슬라이싱에서는 인덱스가 범위를 벗어나도 오류가 발생하지 않는다.
...
이는 슬라이싱이 내부적으로 **가능한 범위로 자동 보정(clamping)** 하기 때문이다.
...
- start, end, step은 모두 생략 가능하다.
- start는 포함되고 end는 포함되지 않는다.
- step이 양수이면 오른쪽으로 이동한다.
- step이 음수이면 왼쪽으로 이동한다.
- start와 end의 방향이 step과 맞지 않으면 결과는 빈 리스트가 된다.
...
# = print(arr[0:3:1])
print(arr[-3:]) 
# = print(arr[-3:len(arr):1])
print(arr[:-3])
# = print(arr[0:-3:1]), step이 기본적으로 1이므로 생략된 start는 0 
print(arr[:-3:-1]) 
# print(arr[len(arr)-1:-3:-1])
print(arr[:-3:-2]) 
# print(arr[len(arr)-1:-3:-2])
print(arr[:-5:-2]) 
# print(arr[len(arr)-1:-5:-2])
print(arr[:3:-1]) 
# print(arr[len(arr)-1:3:-1])
print(arr[0:3:-1]) # Start -> end 방향과 step의 방향이 다름. 빈 리스트 출력 
```
````
`````

````{admonition} 왜 정렬 알고리즘이 이렇게 많은가? 하나만 있으면 되는 거 아닌가?
:class: attention 

상황마다 중요한 기준이 다르기 때문이다. 
- 데이터가 거의 정렬되어 있는지
- 메모리가 어느 정도 있는지
- 안정성이 필요한지
- 평균 속도가 중요한지, 최악의 속도가 중요한지 

이런 조건이 다 다르기 때문에 여러 알고리즘들이 존재한다. 

예를 들어, 버블/삽입 정렬은 데이터가 거의 정렬된 경우 평균 시간 복잡도 O(NlogN)을 가진 퀵 정렬보다 빠를 수 있다. 
````
...
- swap: 위에서 피벗의 오른쪽인 '1'과 pivot '5' 의 위치를 서로 변경 -> ***변경 후에 피벗 데이터는 피벗 오른쪽 인덱스*** 에 위치함. 
...
````{admonition} 퀵 정렬은 왜 최악의 경우 O($N^2$)가 되는가?
:class: attention 

피벗을 기준으로 좌우로 나누는 과정에서 "균등하게" 나눠져야 NlogN이 된다. 
하지만 이미 정렬된 배열처럼 피벗이 계속 한쪽에만 치우치면 분할이 비효율적으로 이루어져 O($N^2$)이 된다. 
````
## 병합 정렬 (Merge sort)
...
````{admonition} 안정적인 정렬 (stable sort)란? 
:class: attention 

안정적인 정렬은 "값이 같은 원소끼리는 원래 순서를 그대로 유지하는 정렬"을 말한다. 예를 들어 "성적이 같은 학생끼리는 기존의 번호 순서가 유지된다" 같은 상황에서 중요하다. 데이터의 의미나 우선순위가 여러 개일 때 매우 유용하다.
````
...
정렬을 **오름차순(ascending order)** 기준으로 수행할 때, 각 알고리즘의 동작 원리는 다음과 같다.

* **선택 정렬 (Selection Sort)**:
  아직 정렬되지 않은 구간(unsorted section)에서 가장 작은 원소를 찾아 **정렬된 구간(sorted section)** 의 **오른쪽 끝에 삽입** 한다.

* **삽입 정렬 (Insertion Sort)**:
  왼쪽 부분 리스트는 항상 정렬되어 있다고 가정하고, 현재 원소를 **왼쪽 정렬 리스트 내 올바른 위치(index)** 에 삽입한다.

* **퀵 정렬 (Quick Sort)**:
  리스트에서 **피벗(pivot)** 을 하나 선택한 뒤, 피벗보다 작은 원소는 왼쪽, 큰 원소는 오른쪽으로 **분할(partitioning)** 하고, 이 과정을 각 부분 리스트에 대해 **재귀적으로 반복**한다.

* **계수 정렬 (Counting Sort)**:
  모든 데이터를 확인하며 **데이터 값과 동일한 인덱스의 카운트 배열(count array)** 값을 1씩 증가시키고, 이를 이용해 정렬된 결과를 생성한다.
````
