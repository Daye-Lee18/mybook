...
|---|---|---|---|---|
|start| 시작인덱스|0 | len(arr) -1 | 포함됨 | 
|end | 끝인덱스 | len(arr) | -1 | 포함안됨 | 
|step | 간격 (양수면 오른쪽, 음수면 왼쪽으로 이동) | 1 | 1 | 방향 결정자 | 

위의 테이블에서 볼 수 있듯이, start와 end가 생략되어있는 경우 step의 부호에 따라서 default로 지정되는 값이 상이하다.
사실 -1, -2...과 같은 음수 인덱스는 len(arr)-1, len(arr)-2와 동일하여, arr[:-3:-1]과 같은 경우, arr[len(arr)-1:-3:-1]이 되고, 이는 n=10인 경우, arr[9:-3:-1] -> arr[9:7:-1]이 되어 계산할 수 있다. 위와 같은 계산 즉, -2를 len(arr)-2로 바꾸는 계산은 ***값의 시작점을 알리는 start 에 대해서만 적용*** 한다. 

그러나 arr[::-1] 은 N=10일 때, arr[len(arr)-1:-1:-1] == arr[9:-1:-1]인데, 이것을 다시 arr[9:9:-1]로 바꾸면 안된다. 즉, end에서 -1은 경계 (중단점)을 의미한다. 즉, 9부터 step=-1 왼쪽으로 가면서 0까지 slicing한다는 의미이다. 

정리 
- step > 0, end가 음수이면 양수로 환산 가능 (-3 -> len(arr) -3 으로 변경 가능) 
- step < 0, end를 마음대로 바꿀 수 없음. (음수 step에서는 end가 멈추어야 하는 경계 (stop boundary) 역할이기 때문)
- start위치에서 음수 인덱스는 양수 환산 가능 

또한 N=10일때, arr[100]은 IndexError가 난다. 이는 정확히 그 칸을 읽어야하기 때문이지만, 슬라이싱 arr[100:200]을 해도 아무 문제 없다. 왜냐면, 슬라이싱은 "실제 가능한 범위로 자동 보정(clamping)" 하기 때문이다. 
