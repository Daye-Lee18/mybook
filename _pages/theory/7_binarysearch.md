...
:Note:
아래함수에서 조건이 `arr[idx]`가 아니라 `arr[idx] != None`인데, 이는 `arr[idx]`만 사용하는 경우 값이 0 일때 None이 되기 때문에 `arr[idx]!=None`일때 node number가 0인 TreeNode를 만들어주는 것이 중요하다. 

또한, `while res and not res[-1]:` 이라고 하면 res[-1] 가 0일때도 삭제되어서 안되므로 `while res and res[-1] == None`이라고 명시해줘야한다.  
