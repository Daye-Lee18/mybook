...
DP is about `state representation`. DP works by </u>breaking a big problem into smaller overlapping subproblems</u>. To reuse solutions, you must define a `state` that fully describes a subproblem. That "state" is exactly what you store in the DP table. 
